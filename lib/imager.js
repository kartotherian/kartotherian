...
function downloadMapdata(domain, title, groups) {
    var mwapi = new MWApi('kartotherian (yurik @ wikimedia)', 'https://' + domain + '/w/api.php');
    var mapdata = [];
    var request = {
        'prop': 'mapdata',
        'mpdlimit': 'max',
    ...
    return mwapi.query(request, undefined, function(result) {
...
module.exports.render = function render(source, format, req, z, scale, handler) {
    ...
    if (format === 'png' && req.query.domain && req.query.title) {
        return BBPromise.all([
            downloadMapdata(req.query.domain, req.query.title, req.query.groups),
            abaculus(params)
