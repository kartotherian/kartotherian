    ...
    return new Promise(function (resolve, reject) {
        ...
        function addJob(idx) {
            ...
            if (options.tiles.length > 0) {
                stream.pause();
                let promise = addJobCallback(_.clone(options)).then(function (v) {
                    stream.resume();
                    return v;
                }).catch(reject);
...
function fileParser(filepath, options, addJobCallback) {
    ...
    return Promise.try(function () {
        // validate options
        options.tiles = [];
        if (checkType(options, 'fileZoomOverride', 'zoom')) {
