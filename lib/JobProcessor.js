...
JobProcessor.prototype.reportProgress = function reportProgress (isDone) {
    ...
    let stats = self.stats,
        job = self.job,
        execTime = (now - self.start) / 1000,
        progress, promise;

    stats.itemsPerSec = execTime > 0 ? Math.round((stats.processed - self.processedAtRestart) / execTime * 10) / 10 : 0;
    ...
    if (!isDone) {
        ...
        if ((now - (self.lastRebalanceTime || self.start)) > (1 + Math.random() * 6) * 60 * 1000) {
            ...
            promise = self.queue.getPendingCountAsync().then(function (pendingCount) {
                if (pendingCount < 10 && stats.estimateHrs > self.minEstimateHrsToBreak) {
                    // next time break if the job is half as long, but not less than 10 minutes
                    self.minEstimateHrsToBreak = Math.max(self.minEstimateHrsToBreak / 2, 1 / 6);
                    // Split current job into 2
                    let newJobs = job.splitJob(2);
                    return self.queue.addJobAsync(newJobs).then(function () {
                        return self.kueJob.saveAsync();
                    }).then(function () {
                        return self.kueJob.inactiveAsync();
    ...
    return (promise || Promise.resolve(false)).then(function (stopProcessing) {
        self.kueJob.progress(progress, job.size, stats);
        self.lastProgressReportTime = now;
        if (stopProcessing) {
            var error = new Err('Job was split');
            delete error.stack; // Make kue's log shorter
            throw error;
        }
    });
...
JobProcessor.prototype.jobProcessorAsync = function() {
    ...
    return self.iterator().then(function (iterValue) {
        ...
        return self.generateTileAsync(iterValue).then(function () {
            ...
            self.stats.index = iterValue.idx + 1;
