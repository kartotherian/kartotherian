...
function JobProcessor(sources, kueJob, metrics, queue) {
    ...
    this.processedAtRestart = stats.processed;
...
JobProcessor.prototype.reportProgress = function reportProgress (isDone) {
    ...
    let job = self.job,
        stats = job.stats,
        execTime = (now - self.jobStart) / 1000,
        progress, promise;
    ...
    if (!isDone) {
        ...
        progress = job.indexToPos(stats.jobIdxFrom) + 1;
        ...
        if ((now - (self.lastRebalanceTime || self.jobStart)) > (1 + Math.random() * 6) * 60 * 1000) {
