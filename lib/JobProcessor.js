...
JobProcessor.prototype.getExistingTilesIterator = function(idxFrom, idxBefore, filterIndex) {
    ...
    return promistreamus.flatten(promistreamus.select(
        this.sequenceToRangesIterator(iterator),
        function (range) {
            return self.getRangeIterator(
                Math.max(job.idxFrom, range[0] * (scale || 1)),
                Math.min(job.idxBefore, range[1] * (scale || 1)),
                filterIndex + 1);
        }));
};
 ...
 */
JobProcessor.prototype.sequenceToRangesIterator = function(iterator) {
