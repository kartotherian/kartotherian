...
core.mapSequentialAsync = function mapSequentialAsync(list, iterator) {
    //
    // FIXME:  should use Promise.each() instead
    //
    return _.reduce(
        list,
        (promise, value, key) => promise
            .then(() => iterator(value, key)),
...
core.loadSource = function loadSource(sourceUri) {
    return core.tilelive.loadAsync(sourceUri).then(handler => {
        ...
        // if (handler.getTileAsync2 === undefined) {
        //     // If the source does not support the new getTileAsync2 interface, inject it
        //
        //
        //     // Annoyingly, Tilesource API has a few functions that break the typical NodeJS callback
        //     // pattern of function(error, result), and instead have multiple results.  For them,
        ...
        //     // See also: http://bluebirdjs.com/docs/api/promise.promisifyall.html#option-multiargs
        //     Promise.promisifyAll(handler, {
        //         filter: name => name === 'getTile' || name === 'getGrid',
        //         multiArgs: true
        //     });
        //     // Promisify the rest of the methods
        //     Promise.promisifyAll(handler);
        //
        //     let getTileFunc = handler.getTileAsync.bind(handler);
        //     handler.getTileAsync2 = function getTileAsync2(opts) {
        //         core.validateGetTileOpts(opts);
        //         return getTileFunc(opts.zoom, opts.x, opts.y);
        //     };
        // }
        // return handler;
    });
