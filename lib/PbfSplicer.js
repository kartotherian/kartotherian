    ...
    if (options.languages) {
        ...
        for (let ind = 0; ind < options.languages.length; ind++) {
            ...
            if (!this.languages.hasOwnProperty(id)) {
                this.languages[id] = ind + 1; // Must be a positive number
            }
        }
    }
}
...
PbfSplicer.prototype.processTile = function processTile(data) {
    let tile = tileCodec.decodeTile(data),
        changed;

    for (const layer of tile.layers) {
        // Optimization - don't process layer if it has no relevant tags
        if (!_.contains(layer.keys, this.multiTag) && !_.contains(layer.keys, this.nameTag)) continue;
        ...
        for (const feature of layer.features) {
            ...
            if (this.languages) {
                feature.tags = this.pickOneLanguage(layer, tags, addValueFunc);
            } else {
                feature.tags = this.jsonTagToMultiple(layer, tags, addValueFunc);
            }
        }
...
PbfSplicer.prototype.jsonTagToMultiple = function jsonTagToMultiple(layer, tags, addValueFunc) {
    ...
    for (let ind = 0; ind < tags.length; ind += 2) {
        ...
        } else if (key !== this.nameTag) {
            addValueFunc(key, value, newTags);
        } else {
            // 'name' is the last fallback, but add it before all others
            addValueFunc(this.multiTag, value, newTags);
        }
    }

    if (jsonTag) {
        // Expand object into multiple tags, prepend the old key to avoid accidental duplications
...
PbfSplicer.prototype.pickOneLanguage = function pickOneLanguage(layer, tags, addValueFunc) {
    ...
    for (let ind = 0; ind < tags.length; ind += 2) {
        ...
        if (!key.startsWith(this.multiTag)) {
            addValueFunc(key, value, newTags);
        } else {
            if (value.tag !== 1) {
                throw new Error('Expecting a tag of type STRING');
