...
PbfSplicer.prototype.pickOneLanguage = function pickOneLanguage(layer, tags, addValueFunc) {
    ...
    const bestValue = langPicker.getResult();
    if (bestValue && (!nameTag || nameTag.value !== bestValue.value)) {
        if (this.combineName) {
            nameTag.value = `${bestValue.value} (${nameTag.value})`;
            addValueFunc(this.nameTag, nameTag, newTags);
        } else {
            // Only add localized name if it is different from the local name
            addValueFunc(this.multiTag, bestValue, newTags);
        }
    } else if (this.combineName) {
        addValueFunc(this.nameTag, nameTag, newTags);
    }
