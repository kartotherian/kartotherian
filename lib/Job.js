...
let _ = require('underscore'),
    ...
    allowedProps = [
        ...
        'generatorId', 'deleteEmpty', 'filters', 'priority', 'tiles', '_encodedTiles'
    ];
...
function Job(opts, stats) {
    ...
    if (core.checkType(this.stats, 'lastCompleteIdx', 'integer', undefined, minFrom, maxBefore - 1)) {
        minFrom = this.stats.lastCompleteIdx + 1;
    }
    core.checkType(this.stats, 'jobIdxBefore', 'integer', maxBefore, minFrom, maxBefore);
...
Job.prototype.moveNextRange = function moveNextRange () {
    ...
    this._assertSimple();
    this._currentRange = this.iterateOverRanges(function(idxFrom, idxBefore) {
 ...
 * @param callback
 * @return {int} new value for _currentRange
 */
Job.prototype.iterateOverRanges = function iterateOverRanges(callback) {
...
Job.prototype.expandJobs = function expandJobs(partsOverride) {
    if (!this.isComplex() && partsOverride === undefined) {
        return [this];
    }
    ...
    for (let zoom = fromZoom; zoom < beforeZoom; zoom ++) {
        ...
        this.iterateOverRanges(function(idxFrom, idxBefore) {
...
/**
 * Call after an index has been complete
 * @param {int} index
 */
...
Job.prototype.isValidIndex = function isValidIndex(index) {
    return index < this.stat.jobIdxBefore &&
