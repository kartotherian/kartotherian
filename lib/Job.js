...
var allowedProps = [
    'zoom', 'x', 'y', 'idxFrom', 'idxBefore', 'fromZoom', 'beforeZoom', 'parts', 'sources', 'storageId', 'generatorId',
    'deleteEmpty', 'filters', 'priority', 'size', 'tiles', '_encodedTiles'
];
...
function Job(opts) {
    ...
    if (core.checkType(this, 'filters', 'object')) {
        ...
        _.each(this.filters, function(filter, ind, all) {
            ...
            var hasDateFrom = core.checkType(filter, 'dateFrom', '[object Date]'),
...
Job.prototype.expandJobs = function expandJobs() {
    ...
    _.range(fromZoom, beforeZoom).map(function (zoom) {
        var mult = Math.pow(4, Math.abs(zoom - self.zoom)),
            opts = _.clone(self),
            size = 0,
            tiles = [],
            lastRange = undefined;
        ...
        for (var rangeId = self.currentRange || 0; rangeId < self.tiles.length; rangeId++) {
            var v = self.tiles[rangeId],
...
Job.prototype._setJobTitle = function _setJobTitle() {
    ...
    if (this.size === 0) {
        ...
        var xy = core.indexToXY(idxFrom);
        this.title += util.format('; 1 tile at [%d,%d] (idx=%d)', xy[0], xy[1], idxFrom);
    } else {
        var xyFrom = core.indexToXY(idxFrom);
        var xyLast = core.indexToXY(idxBefore - 1);
