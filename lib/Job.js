...
function Job(opts, stats) {
    ...
    let self = this;
    _.each(allowedProps, prop => {
        if (opts.hasOwnProperty(prop)) {
            self[prop] = opts[prop];
        }
    });

    checkType(this, 'fromZoom', 'zoom');
    checkType(this, 'beforeZoom', 'zoom', undefined, this.fromZoom);
    checkType(this, 'parts', 'integer', undefined, 1, 1000);
    if (this.parts === 1) {
        delete this.parts;
    }
    ...
    checkType(this, 'storageId', 'string', true, 1);
    checkType(this, 'generatorId', 'string', true, 1);
    checkType(this, 'zoom', 'zoom', true);
    checkType(this, 'deleteEmpty', 'boolean');
    checkType(this, 'keepJob', 'boolean');
    ...
    if (this.x !== undefined || this.y !== undefined ) {
        ...
        checkType(this, 'x', 'integer', true);
        checkType(this, 'y', 'integer', true);
        if (!qidx.isValidCoordinate(this.x, zoom) || !qidx.isValidCoordinate(this.y, zoom)) {
            throw new Err('Invalid x,y coordinates for the given zoom');
        }
        this.idxFrom = qidx.xyToIndex(this.x, this.y);
        this.idxBefore = this.idxFrom + 1;
        delete this.x;
        delete this.y;
    }

    checkType(this, 'idxFrom', 'integer', undefined, 0, maxCount);
    checkType(this, 'idxBefore', 'integer', undefined, this.idxFrom || 0, maxCount);
    if (this.idxFrom !== undefined || this.idxBefore !== undefined) {
    ...
    checkType(this, 'tiles', 'array');
    checkType(this, '_encodedTiles', 'array');

    checkType(this, 'sources', 'object');

    // priority can be both a number and a string like 'highest', so don't check
    this.priority = this.priority || 0;

    if (checkType(this, 'filters', 'object')) {
        ...
        _.each(this.filters, function(filter, ind, all) {
            ...
            if (checkType(filter, 'zoom', 'integer') && !this.isComplex()) {
                if (filter.zoom < 0) {
                    filter.zoom = this.zoom + filter.zoom;
                }
                checkType(filter, 'zoom', 'zoom',
                    ind < all.length - 1,
                    ind === 0 ? 0 : all[ind - 1].zoom + 1,
                    this.zoom);
            }
            let hasDateFrom = checkType(filter, 'dateFrom', '[object Date]'),
                hasDateBefore = checkType(filter, 'dateBefore', '[object Date]');
            ...
            checkType(filter, 'missing', 'boolean');
            checkType(filter, 'sourceId', 'string', false, 1);
        }, this);
    }
    ...
    if (checkType(stats, 'lastCompleteIdx', 'integer', undefined, minFrom, maxBefore - 1)) {
        minFrom = stats.lastCompleteIdx + 1;
    }
    checkType(stats, 'jobIdxBefore', 'integer', maxBefore, minFrom, maxBefore);
    this.stats = stats;
...
Job.prototype.moveNextRange = function moveNextRange () {
    ...
    this._assertSimple();
    this._currentRange = this.iterateOverRanges(this._currentRange, (idxFrom, idxBefore) => {
...
Job.prototype.splitJob = function splitJob(parts) {
    ...
    if (subJobs.length > 1) {
        ...
        subJobs.shift();
        this.size -= subJobs.reduce((acc, sj) => acc + sj.size, 0);
        this._setJobTitle();
...
Job.prototype._encodeTileList = function _encodeTileList() {
    let size = 0,
        ...
        maxCount = Math.pow(4, this.zoom),
        result = [],
        checkIdx = (v, min, max) => Number.isInteger(v) && v > min && v < max;

    if (!_.all(this.tiles, v => {
...
Job.prototype._setJobTitle = function _setJobTitle() {
    ...
    if (this.size === 0) {
        ...
        let xy = qidx.indexToXY(idxFrom);
        this.title += util.format('; 1 tile at [%d,%d] (idx=%d)', xy[0], xy[1], idxFrom);
    } else {
        let xyFrom = qidx.indexToXY(idxFrom),
            xyLast = qidx.indexToXY(idxBefore - 1);
