...
CassandraStore.prototype.getTile = function(z, x, y, callback) {
    var self = this;
    return this.queryTileAsync({zoom: z, idx: core.xyToIndex(x, y, z)}).then(function(row) {
        if (row) {
...
CassandraStore.prototype.putInfo = function(data, callback) {
    // hack: Store source info under zoom 0 with ID 1 (which is invalid for tiles)
    return this._storeDataAsync(0, 1, new Buffer(JSON.stringify(data))).nodeify(callback)
};
