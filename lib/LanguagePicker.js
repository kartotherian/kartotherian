const langFallbacks = require('./fallbacks.json');
...
function LanguagePicker(lang = 'en', config = {}) {
  ...
  if (langFallbacks[lang]) {
    fallbacks = Array.isArray(langFallbacks[lang]) ?
      langFallbacks[lang] : [langFallbacks[lang]];
  }
...
LanguagePicker.prototype.newProcessor = function newProcessor() {
  ...
  return {
    addValue(lang, value) {
      if (
        fallbacks.indexOf(lang) > -1 &&
        fallbacks.indexOf(lang) <= fallbacks.indexOf(bestChoiceLang)
      ) {
