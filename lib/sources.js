...
Sources.prototype._loadSourceAsync = function(src, sourceId) {
    ...
    return Promise.try(function () {
        ...
        let uri = core.normalizeUri(src.uri);
        ...
        src.getHandler = function () {
            return handler;
        };
    }).catch(function (err) {
        err.message = 'Unable to load source "' + sourceId + '"\n' + (err.message || '');
        core.log('error', err);
...
Sources.prototype._setXmlParameters = function(newValues, xmlParams, xmlFile) {
    let self = this;
    _.each(newValues, function (value, name) {
        let param = self._xmlParamByName(xmlParams, name, xmlFile);
        //noinspection JSAccessibilityCheck
...
Sources.prototype._loadXmlAsync = function(src, uri) {
    ...
    let promise = fs.readFileAsync(xmlFile, 'utf8');
    ...
    if (src.xmlSetAttrs || src.xmlSetParams || src.xmlLayers || src.xmlExceptLayers || src.xmlSetDataSource) {
        promise = promise.then(function (xml) {
            ...
            let layerFunc = getLayerFilter(src);
            if (layerFunc) {
                let result = [];
            ...
            if (core.checkType(src, 'xmlSetDataSource', 'object')) {
                let dataSources = src.xmlSetDataSource;
                ...
                _.each(dataSources, function(ds) {
                    ...
                    doc.eachChild(function (xmlLayer) {
                        ...
                        if (!xmlParams) {
                            core.log('warn', '<Datasource> xml element was not found in layer %s in %s', xmlLayer.attr.name, xmlFile);
                            return;
                        }
                        if (conditions) {
                        ...
                        core.log('trace', 'Updating layer ' + xmlLayer.attr.name);
...
Sources.prototype.getSourceById = function(sourceId, dontThrow, allowDisabled) {
    ...
    if (!allowDisabled && source.isDisabled) {
        if (dontThrow) return undefined;
        throw new Err('Source "%s" is disabled, possibly due to loading errors', sourceId);
    }
...
function getLayerFilter(src) {
    let include = core.checkType(src, 'xmlLayers', 'string-array'),
        exclude = core.checkType(src, 'xmlExceptLayers', 'string-array');
    if (!include && !exclude) {
        return undefined;
    }
    if (include && exclude) {
    ...
    let layers = include ? src.xmlLayers : src.xmlExceptLayers;
