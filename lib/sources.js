...
function initAsync(app, tilelive, npmResolver, serviceRootDir) {
    ...
    return localOrExternalDataAsync(app.conf.variables, serviceRootDir, 'variables')
        .then(function (vars) {
            ...
            return _.reduce(sources, function (promise, src, key) {
            ...
            }, BBPromise.resolve()).return(sources);
        }).then(function (sources) {
...
function getParameterByName(xmlParams, name, xmlFile) {
    ...
    if (!param || param.name !== 'Parameter') {
        throw new Error(util.format('<Parameter name="%s"> xml element was not found in %s', name, xmlFile));
    }
    return param;
}
    ...
    if (src.xmlSetParams || src.xmlLayers || src.xmlExceptLayers || src.xmlSetDataSource) {
        promise = promise.then(function (xml) {
            ...
            if (core.checkType(src, 'xmlSetParams', 'object')) {
                ...
                if (!xmlParams) {
                    throw new Error('<Parameters> xml element was not found in ' + src.xml);
                }
            ...
            if (core.checkType(src, 'xmlSetDataSource', 'object')) {
                ...
                _.each(dataSources, function(ds) {
                    if (typeof ds !== 'object' || Array.isArray(ds)) {
                        throw new Error('XmlLoader: xmlSetDataSource must be an object');
                    }
                    ...
                    doc.eachChild(function (xmlLayer) {
                        ...
                        if (!xmlParams) {
                            console.log(util.format('<Datasource> xml element was not found in layer %s in %s', xmlLayer.attr.name, src.xml));
                            return;
                        }
                        if (conditions) {
...
function getLayerFilter(src) {
    ...
    if (include && exclude) {
        throw new Error('XmlLoader: it may be either xmlLayers or xmlExceptLayers, not both');
    }
    var layers = include ? src.xmlLayers : src.xmlExceptLayers;
    if (!Array.isArray(layers)) {
        throw new Error(util.format('XmlLoader xmlLayers/xmlExceptLayers must be a string or an array of strings'));
    }
...
function resolveValue(value, npmResolver) {
    ...
    if (key.length !== 1) {
        throw new Error('Value must be an object with one key-value pair');
    }
    key = key[0];
    value = value[key];
    ...
    switch (key) {
        ...
        default:
            throw new Error(util.format('Value type %s is not recognized', key));
    }
}
