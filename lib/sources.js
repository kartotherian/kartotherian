...
Sources.prototype.loadVariablesAsync = function(variables) {
    var self = this;
    return self._localOrExternalDataAsync(variables, 'variables').then(function (variables) {
...
Sources.prototype.loadSourcesAsync = function(sources) {
    var self = this;
    return self._localOrExternalDataAsync(sources, 'sources').then(function (sources) {
        self._sources = _.extend(self._sources, sources);
...
Sources.prototype._loadSourceAsync = function(src, sourceId) {
    ...
    return BBPromise.try(function () {
        ...
        return core.loadSource(uri, self._tilelive);
    }).then(function (handler) {
        self._handlers[sourceId] = handler;
    });
};
    ...
    return BBPromise.try(function () {
        ...
        values = _.map(values, function (value) {
            if (typeof value === 'string') {
                ...
                self._log('info', 'Loading ' + name + ' from external file ' + path);
                return fs
                    .readFileAsync(path)
                    .then(yaml.safeLoad);
...
Sources.prototype._loadXmlAsync = function(src, uri) {
    var self = this;
    var xmlFile = self._resolveValue(src.xml, 'xml');
...
Sources.prototype.getHandlerById = function(sourceId, dontThrow) {
    if (this.getSourceById(sourceId, dontThrow)) {
        if (!this._handlers.hasOwnProperty(sourceId)) {
            if (dontThrow) return undefined;
            throw new Err('Unknown handler %s', sourceId);
        }
        return this._handlers[sourceId];
    }
    return undefined;
};
...
Sources.prototype._getVariable = function(name) {
    core.checkType(this._variables, name, 'string', true);
