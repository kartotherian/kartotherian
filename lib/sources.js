...
Sources.prototype.init = function(variables, sources) {
    let self = this;
    return Promise.try(() =>
        self.loadVariablesAsync(self._localOrExternalDataAsync(variables, 'variables'))
    ).then(() =>
        self.loadSourcesAsync(self._localOrExternalDataAsync(sources, 'sources'))
    ).return(self);
};
...
Sources.prototype.loadSourcesAsync = function(sources) {
    ...
    return Promise.resolve(sources).then(sources => {
        ...
        return core.mapSequentialAsync(sources, (src, key) => self._loadSourceAsync(src, key));
    });
};
