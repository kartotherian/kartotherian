...
function reportRequestError(err) {
...
function requestHandler(req, res, next) {
    ...
    }).catch(reportRequestError).catch(next);
}
...
module.exports.init = function(opts) {
    ...
    // /marker/pin-m-cafe-7e7e7e@2x.png
    router.get('/marker/:base([\\w]+),:size([sml]),:symbol([-\\w]+),:color([a-f0-9]+).png', markerHandler);
    router.get('/marker/:base([\\w]+),:size([sml]),:symbol([-\\w]+),:color([a-f0-9]+)@:scale([\\.\\d]+)x.png', markerHandler);
