...
module.exports.checkType = function(obj, field, expType, mustHave, min, max) {
    ...
    if (type === 'number' && expType === 'integer') {
        if (value % 1 !== 0) {
            throw new Err('Invalid %s param: %d is given, but %s expected', field, value, expType);
        }
        type = 'integer';
    }
    if (type !== expType) {
        throw new Err('Invalid %s param type %s given, but %s expected', field, type, expType);
    }
    switch (expType) {
        case 'number':
        case 'integer':
